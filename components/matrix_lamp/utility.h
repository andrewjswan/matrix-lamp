...
static void drawPixelXY(coord_t x, coord_t y, const CRGB &color) {
  if (x < 0 || x >= (coord_t) WIDTH || y < 0 || y >= (coord_t) HEIGHT)
    return;
  leds[XY(x, y)] = color;
}
...
inline void blur1d(uint16_t numLeds, uint8_t blur_amount) {
...
inline void blurRows(uint8_t width, uint8_t height, uint8_t blur_amount) {
  uint8_t keep = 255 - blur_amount;
  uint8_t seep = blur_amount >> 1;
...
inline void blurColumns(uint8_t width, uint8_t height, uint8_t blur_amount) {
  uint8_t keep = 255 - blur_amount;
  uint8_t seep = blur_amount >> 1;
...
inline void blur2d(uint8_t width, uint8_t height, uint8_t blur_amount) {
...
static void ledsClear() { fillAll(CRGB(0, 0, 0)); }

// стандартные функции библиотеки LEDraw от @Palpalych (для адаптаций его эффектов)
static void blurScreen(uint8_t blur_amount) { blur2d(WIDTH, HEIGHT, blur_amount); }
...
static CRGB makeDarker(const CRGB &color, uint8_t howMuchDarker) {
  CRGB newcolor = color;
...
// ------------------------------------------------
static uint8_t wrapX(int16_t x) { return (x % (int16_t) WIDTH + (int16_t) WIDTH) % (int16_t) WIDTH; }

static uint8_t wrapY(int16_t y) { return (y % (int16_t) HEIGHT + (int16_t) HEIGHT) % (int16_t) HEIGHT; }
...
// ------------------------------------------------
inline void drawPixelXYFseamless(float x, float y, const CRGB &color) {
...
static void drawCircle(int x0, int y0, int radius, const CRGB &color) {
  ...
  const bool completely_inside =
      (x0 - radius >= 0) && (x0 + radius < (int) WIDTH) && (y0 - radius >= 0) && (y0 + radius < (int) HEIGHT);
...
// ------------------------------------------------
inline void drawRec(uint8_t startX, uint8_t startY, uint8_t endX, uint8_t endY, uint32_t color) {
...
// ------------------------------------------------
inline void drawRecCHSV(uint8_t startX, uint8_t startY, uint8_t endX, uint8_t endY, const CHSV &color) {
  for (uint8_t y = startY; y < endY; y++) {
    for (uint8_t x = startX; x < endX; x++) {
      drawPixelXY(x, y, color);
    }
...
// ------------------------------------------------
// альтернативный градиент для ламп собраных из лент с вертикальной компоновкой
static void gradientHorizontal(uint8_t startX, uint8_t startY, uint8_t endX, uint8_t endY, uint8_t start_color,
                               uint8_t end_color, uint8_t start_br, uint8_t end_br, uint8_t saturate) {
  if (startX == endX) {
    endX++;
  }
  if (startY == endY) {
...
// ------------------------------------------------
static void gradientVertical(uint8_t startX, uint8_t startY, uint8_t endX, uint8_t endY, uint8_t start_color,
                             uint8_t end_color, uint8_t start_br, uint8_t end_br, uint8_t saturate) {
  if (startX == endX) {
    endX++;
  }
...
static void gradientDownTop(uint8_t bottom, CHSV bottom_color, uint8_t top, const CHSV &top_color) {
  ...
  if (ORIENTATION < 3U || ORIENTATION == 7U) {
    ...
    fill_gradient(leds, start_p, top_color, end_p, bottom_color, SHORTEST_HUES);
  } else {
    ...
    fill_gradient(leds, start_p, bottom_color, end_p, top_color, SHORTEST_HUES);
  }
}
