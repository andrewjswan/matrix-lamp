...
void MatrixLamp::on_shutdown() { free_leds(); }

#if defined(MATRIX_LAMP_USE_DISPLAY)
void MatrixLamp::set_display(addressable_light::AddressableLightDisplay *display) { this->display = display; }

void MatrixLamp::add_icon(MatrixLamp_Icon *icon) {
  this->icons[this->icon_count] = icon;
  ESP_LOGD(TAG, "Add Icon No.: %d name: %s frame_duration: %d ms", this->icon_count, icon->name.c_str(),
           icon->frame_duration);
  this->icon_count++;
}
...
void MatrixLamp::set_scale(template_::TemplateNumber *scale) { this->scale = scale; }  // set_scale()

void MatrixLamp::set_speed(template_::TemplateNumber *speed) { this->speed = speed; }  // set_speed()

void MatrixLamp::reset_current_effect() { currentMode = MODE_AMOUNT; }  // reset_current_effect()
...
void MatrixLamp::set_custom_effect_mode(uint8_t mode) { custom_eff = mode; }  // set_custom_effect_mode()
...
uint8_t MatrixLamp::get_effect_speed() { return this->get_speed_for_effect(currentMode); }  // get_effect_speed()
...
uint8_t MatrixLamp::get_effect_scale() { return this->get_scale_for_effect(currentMode); }  // get_effect_scale()
...
#if defined(RANDOM_SETTINGS_IN_CYCLE_MODE)
// Get random settings
bool MatrixLamp::get_random_settings() { return random_settings; }
...
void MatrixLamp::ShowFrame(uint8_t CurrentMode, esphome::Color current_color, light::AddressableLight *p_it) {
  ...
  if (trigger) {
    ...
    for (auto *t : on_effect_start_triggers_) {
      t->process((uint8_t) currentMode, (uint8_t) modes[currentMode].Brightness, (uint8_t) modes[currentMode].Speed,
                 (uint8_t) modes[currentMode].Scale);
    }
  }
#endif
...
void MatrixLamp::set_brightness(int32_t value) {
  if (value < 256) {
    ...
    ESP_LOGI(TAG, "Display: set_brightness %d => %.2f %%", (int) value, 100 * br);
  }
}
